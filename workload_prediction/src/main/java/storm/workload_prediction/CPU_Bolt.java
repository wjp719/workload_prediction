...
public class CPU_Bolt implements IRichBolt {
	...
	public void execute(Tuple input) {
		...
		try {
			...
			String url = "";
			String url_pre = "http://166.111.143.220:8777/v2/meters/cpu_util/statistics?";
			...
			for (int j = 0; j < period_num; j++) {
				...
				url = url_pre + condition_id + id + "&" + time_condition0 + "&" + time_condition1 + "&"
						+ conditon_perod;
			...
			// 查询当天历史数据
			if (avg[0] != -1) {// avg=说明没有查询到关于该指标的历史数据，无法计算平均值
